...
function switchNav(tab,pushState=true){
  ...
  if(tab==='duvar'&&!eksBasliklar.length)loadBasliklar();
...
function renderDuvarSidebar(){
  const el=document.getElementById('duvarSidebarList');
  if(!el)return;
...
function sidebarOpenBaslik(id,baslik){
  switchNav('kesit');
  openBaslik(id,baslik);
}
